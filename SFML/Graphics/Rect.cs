	...
	namespace Graphics {
		...
		public struct IntRect : IEquatable<IntRect> {
			...
			public override int GetHashCode() {
				return unchecked((int)((uint)Left ^ (((uint)Top << 13) | ((uint)Top >> 19)) ^ (((uint)Width << 26) | ((uint)Width >> 6)) ^ (((uint)Height << 7) | ((uint)Height >> 25))));
			}
		...
		public struct FloatRect : IEquatable<FloatRect> {
			...
			public static explicit operator IntRect(FloatRect r) {
				return new IntRect((int)r.Left, (int)r.Top, (int)r.Width, (int)r.Height);
			}
